...
if (isset($_GET["kode"])) {
  ...
  // pengikut 2
  $data_pengikut2 = $conn->query("SELECT pegawai.Nama as 'pengikut' FROM `spt` 
    inner join pegawai on  pegawai.IdPegawai = spt.kepada3 where IdSPT  ='{$_GET['kode']}'")->fetch_assoc();
  // pengikut 3
  $data_pengikut3 = $conn->query("SELECT pegawai.Nama as 'pengikut' FROM `spt` 
    inner join pegawai on  pegawai.IdPegawai = spt.kepada4 where IdSPT  ='{$_GET['kode']}'")->fetch_assoc();
  ...
  if (empty($data_pengikut)) {
    $pengikut = "value='Tidak ada' readonly"; // beda query
  } else {
    $pengikut = "value='" . $data_pengikut['pengikut'] . "' readonly"; // beda query
  }
  if (empty($data_pengikut2)) {
    $pengikut2 = "value='Tidak ada' readonly"; // beda query
  } else {
    $pengikut2 = "value='" . $data_pengikut['pengikut'] . "' readonly"; // beda query
  }
  if (empty($data_pengikut3)) {
    $pengikut3 = "value='Tidak ada' readonly"; // beda query
  } else {
    $pengikut3 = "value='" . $data_pengikut3['pengikut'] . "' readonly"; // beda query
  }
  ...
  $button = "Ubah";
  // cek post submit
  if (isset($_POST['tanggalB']) && isset($_POST['maksud']) && isset($_POST['alat']) && isset($_POST['berangkat'])) {
    mysqli_query($conn, "UPDATE `sppd` SET `IdSPPD`={$_POST['id']},`TanggalBerangkat`='{$_POST['tanggalB']}',`Maksud`='{$_POST['maksud']}',`Angkut`='{$_POST['alat']}',`TempBerangkat`='{$_POST['berangkat']}',
      ...
      `TanggalKembali`='{$_POST['tanggalK']}',`Biaya`='{$_POST['biaya']}' WHERE `IdSPPD`='{$_GET['kode']}'");
  ...
  echo $_POST['id'];
  echo $_POST['tanggalB'];
  mysqli_query($conn, "INSERT INTO `sppd`(`IdSPPD`, `TanggalBerangkat`, `Maksud`, `Angkut`, `TempBerangkat`, `TempTujuan`, `Lama`, `Instansi`, `KodeRek`, `Keterangan`, `pejabatBerwenang`, `TanggalKembali`, `Biaya`, `IdSPT`) 
    VALUES (null, '{$_POST['tanggalB']}', '{$_POST['maksud']}', '{$_POST['alat']}', '{$_POST['berangkat']}', '{$_POST['tujuan']}', {$_POST['lama']}, '{$_POST['instansi']}', '{$_POST['rekening']}', '{$_POST['ket']}', '{$_POST['berwenang']}', '{$_POST['tanggalK']}', '{$_POST['biaya']}', '{$_POST['id']}')");
    ...
    <div id="content" class="p-4 p-md-5">
      ...
      <form class="row g-3" action="">
        ...
        <div class="col-md-6">
          ...
          <div id="nomor_ganti">
            <input id="search" list="nomor" class="form-control" id="nomor" name="nomor" <?= $nomorS ?>>
          </div>
          <datalist id="nomor">
            <?php
            $nomor_spt = $conn->query("SELECT * from spt where verifikasi = 1");
            while ($spt = $nomor_spt->fetch_assoc()) :

            ?>
              <option class="optionID" id="<?= $spt['IdSPT'] ?>" data-tanggal="<?= $spt['Tanggal'] ?>" data-dasar="<?= $spt['Dasar'] ?>" data-nomor="<?= $spt['IdSPT'] ?>" value="<?= $spt['NomorS'] ?>"></option>
            <?php endwhile; ?>
          </datalist>
        </div>
        <div class="col-md-6">
          <label for="">Dasar</label>
          <input class="form-control" name="" id="dasar" readonly></input>
        </div>
      ...
      <form class="row g-3" method="POST" id="tampil">
        ...
        <div class="col-md-6">
          ...
          <input type="date" class="form-control" id="tanggalB" name="tanggalB" min="" <?= $tanggalB ?> required>
        </div>
      ...
      <div class="col-md-6">
        <br>
        <button id="submit" type="submit" class="btn btn-primary"><?= $button ?></button>
      </div>
      ...
      <script type="text/javascript">
        ...
        $(document).ready(function() {
          $('#search').on('keyup', function() {
            // bandingkan value input dengan optionnya, kemudian ambil attribute data-nomor
            var search;
            var dasar;
            var min_tanggal
            $('.optionID').each(function() {
              if ($(this).val() == $('#search').val()) {
                search = $(this).attr('data-nomor');
                dasar = $(this).attr('data-dasar');
                min_tanggal = $(this).attr('data-tanggal');
                // console.log('ok');
              }
            });
            console.log(search);
            // ganti input id spt, dasar spt
            $('#id').val(search);
            $('#dasar').val(dasar);

            console.log(min_tanggal);
            ...
            $.ajax({
              ...
              success: function(data) {
                $('#tampil').html(data);
                $('#tanggalB').attr('min', min_tanggal);
                $('#tanggalK').attr('min', min_tanggal);
      ...
      <script>
        $(document).ready(function() {
          $(document).on('click', '#submit', function() {
            for (const el of document.getElementById('tampil').querySelectorAll("[required]")) {
